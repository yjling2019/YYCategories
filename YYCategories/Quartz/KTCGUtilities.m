...
CGContextRef KTCGContextCreateARGBBitmapContext(CGSize size, BOOL opaque, CGFloat scale) {
...
CGContextRef KTCGContextCreateGrayBitmapContext(CGSize size, CGFloat scale) {
    size_t width = ceil(size.width * scale);
    size_t height = ceil(size.height * scale);
    if (width < 1 || height < 1) return NULL;
...
CGAffineTransform KTCGAffineTransformGetFromPoints(CGPoint before[_Nullable 3], CGPoint after[_Nullable 3]) {
...
CGAffineTransform KTCGAffineTransformGetFromViews(UIView *from, UIView *to) {
    ...
    after[0] = [from kt_convertPoint:before[0] toViewOrWindow:to];
    after[1] = [from kt_convertPoint:before[1] toViewOrWindow:to];
    after[2] = [from kt_convertPoint:before[2] toViewOrWindow:to];
    
    return KTCGAffineTransformGetFromPoints(before, after);
}
...
CGRect KTCGRectFitWithContentMode(CGRect rect, CGSize size, UIViewContentMode mode) {
